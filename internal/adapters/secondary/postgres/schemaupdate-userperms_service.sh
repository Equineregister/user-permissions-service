...
# Script to run update a Tenant's userperms_service database schema (DDL) NOT data changes
...
function show_help {
    ...
    echo -n "-d|--database <database> -m|--migrationfile <migrationfile> "
    echo    ""
    echo    ""
...
if [ ! -f "migrations/${MIGRATIONFILE}" ]; then
    echo "Cannot find MIGRATIONFILE: migrations/$MIGRATIONFILE"
    exit 1
fi


if [[ $(head -1 "migrations/$MIGRATIONFILE") == *$'\r' ]]; then 
    echo "Migration file: migrations/$MIGRATIONFILE contains CR please convert to UNIX line endings"
    exit 1
fi 
...
#TODO:  Do we check for a "-revert" version of the migration file before running??
...
if [[ $MIGRATIONFILE_NUMBER -eq 1 ]]; then 
    MIGRATIONDB_NUMBERSTRING="0000"
    MIGRATIONDB_NUMBER=0
else

    MIGRATIONDB_NUMBERSTRING=`echo "SELECT current_schema_number FROM public.db_schema_number;" \
...
MIGRATIONFILE_SQL=`cat migrations/$MIGRATIONFILE`

MIGRATION_SQL=$(cat << EOF
BEGIN;
$MIGRATIONFILE_SQL
UPDATE public.db_schema_number SET current_schema_number='$MIGRATIONFILE_NUMBERSTRING';
...
NEW_MIGRATIONDB_NUMBER=`echo "SELECT current_schema_number FROM public.db_schema_number;" \
                        | psql -v ON_ERROR_STOP=1 -qAt -h $DBHOSTNAME -U $USERNAME -d $DATABASE`
test_psql_exitcode $?
